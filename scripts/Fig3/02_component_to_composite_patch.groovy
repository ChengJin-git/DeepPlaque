    ...
    println channelNames 
       ...
       // extract ID and imageID from the filename         
        def name = GeneralTools.getNameWithoutExtension(imageData.getServer().getMetadata().getName())
            def idIndex = name.indexOf("_Scan")
            def ID = name.substring(0,idIndex)
            def cropIndex = name.indexOf("_component")
            def imageID = name.substring(0,cropIndex)

/******* User inputs****************************************************************/        

    // By default, DAPI channel is in blue colour 
    def DAPIch = channels[0]
        DAPIch.setLUTColor(0, 0, 255)
        
    // Define Abeta channel by index in channel list (e.g. C1 has index 0, C4 has index 4)
    def AbetaChannelIndex = channels[4]
    ...
    def AbetaChannelIndexShow2 = channels[0,4]
    
    // Define output folder paths 
    boolean exportOriginalPxl = true
    boolean exportBBOX = false
    boolean exportAbetaRaw = false 
//    Create a 'colourPairList' folder under outputParentFolder for storing the exported TIFF images
    def outputParentFolder = "/path/to/output/folder"  // TODO: Set your output folder path
...
/*********************************************************************************/        
    ...
    if (exportOriginalPxl==true) {
        ...
        for (colour in AbetaColourList) {
            ...
            def OrginialImgPath = buildFilePath(outputDir, subfolderName, imageID + "_composite_image.tif")
...
//        server.display(displayLatest)
        ...
        def rawAbetaImgPath = buildFilePath(outputDir, "whiteAbeta", imageID + "_composite_image.tif")
