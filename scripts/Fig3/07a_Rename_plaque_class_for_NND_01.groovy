...
// Run the script 08a_MeasurementExporter_annotation_byClass before running this script to obatin the class of each DL-predicted plaque

         def boundary = getAnnotationObjects().findAll {it.getLevel() == 2}
                boundary.each { it ->
                    it.setPathClass(getPathClass("abeta_DL"))
                    it.setLocked(true)
                }  
                resolveHierarchy()
               fireHierarchyUpdate()